...
public class UserLoginAction implements ActionUser {
	...
	public ActionForwardUser execute(HttpServletRequest request, HttpServletResponse response) throws Exception {
		...
		// 회원여부 판단하는 메서드 생성 - int idCheck(user_email,user_pw);
		int check = udao.idCheck(user_email, user_pw);
		// 0 - "비밀번호 오류" alert창
		// -1 - "아이디가 없음" alert창
		// 1 - "로그인 처리"
		...
		if (check == 0) {
			...
			//out.print("  history.back(); ");
			out.print(" location.href='./UserLogin.com'; ");
			out.print("</script>");
            // out 객체의 자원 해제 
			out.close();
